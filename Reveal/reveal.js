customElements.define(
	'reveal-dialog',
	class FoundationishReveal extends HTMLElement {
		/**
		 *  Fields
		 */
		...
		render() {
			...
			let $dialog = $trigger ? this.querySelector('#' + $trigger.getAttribute('data-open')) : false;
			...
			this.addEventListener('click', (event) => {
				...
				let displayHandler = () => {
					...
					// Open modal
					if (!$dialog.open && $control.getAttribute('data-open') === $dialog.id) {
    ...
    */
		...
		#closeDialog($dialog, customProp = '--reveal-display-timing') {
			...
			const fadeoutDuration = getTimingProp(customProp);

			// Only run if the dialog is open
			if (!$dialog.open || $dialog.classList.contains('closing')) {
				return;
			}
			...
			fallbackTimer = setTimeout(finishClose, fadeoutDuration + 50);
		}
    ...
    */
		...
		emitBeforeToggleEvent() {
			this.dispatchEvent(
				new CustomEvent('reveal:before-toggle', {
					bubbles: true,
